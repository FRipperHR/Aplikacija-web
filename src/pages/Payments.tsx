...
import { CreditCard, Plus, Search, Trash2, Calendar, Tags, Edit2 } from 'lucide-react';
...
export default function Payments() {
  const { state, addPayment, updatePayment, deletePayment } = useApp();
  ...
  const payments = state.payments.filter(p => 
    p.type === 'general' && p.name.toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  return (
    <div className="space-y-8">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-slate-900 tracking-tight">Uplate</h1>
          <p className="text-slate-500 mt-1 font-medium">Pratite sve uplate i troškove osim kredita</p>
        </div>
        <button 
      ...
      <div className="flex flex-col md:flex-row gap-4">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full bg-white border border-slate-200 rounded-2xl py-3.5 pl-12 pr-4 text-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all shadow-sm"
          />
        </div>
      </div>
          ...
          <tbody className="divide-y divide-slate-50">
            {payments.length === 0 ? (
              ...
              payments.map((p) => {
                ...
                return (
                  <motion.tr layout key={p.id} className="hover:bg-slate-50 transition-colors group">
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="relative w-full max-w-lg bg-white rounded-3xl shadow-2xl p-8">
              <h2 className="text-2xl font-bold text-slate-900 mb-6">{editingId ? 'Uredi uplatu' : 'Nova uplata'}</h2>
              <form onSubmit={handleSubmit} className="space-y-6">
                <div>
                  <label className="block text-sm font-bold text-slate-700 mb-2">Opis uplate</label>
                  <input required value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all font-medium" />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input 
                      ...
                      className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all font-black" 
                    />
                  </div>
                  <div>
                    <label className="block text-sm font-bold text-slate-700 mb-2">Kategorija</label>
                    <select value={formData.categoryId} onChange={e => setFormData({...formData, categoryId: e.target.value})} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all">
                ...
                <div>
                  ...
                  <input type="date" value={formData.date} onChange={e => setFormData({...formData, date: e.target.value})} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all" />
                </div>
